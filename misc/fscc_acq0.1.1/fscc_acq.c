...
//  char dataz[arg.o.acqsize]; // why?
  ...
  while (*arg.running) {
    ...
    e = fscc_read_with_timeout(h, dataz, arg.o.acqsize, &bytes_read, 1000);
	printf(" read %u bytes.\n", bytes_read);
    if (e != 0) {
      fscc_disconnect(h);
	...
	fscc_log("Incomplete acquisition!  Got %ui bytes.", bytes_read);
	fprintf(stderr, "Error writing: Got %ui bytes.\n", bytes_read);
      }
    }
